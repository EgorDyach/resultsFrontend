import {PatchParticipant} from "../Server/PatchParticipant.js";
import {serverAddParticipant} from "../Server/getResults.js";
import {useState} from 'react';

export default function ChangeForm({e, onClose}) {
    const [valueName, setValueName] = useState(e.name);
    const [valueProgram, setValueProgram] = useState(e.program);
    const [valueVotes, setValueVotes] = useState(e.votes);
    return (
        <div className={"modalForm"}>
            <h2 class={"modalFormTitle"}>Изменить участника "{e.name}"</h2>
            <div className={"changeFormWrapper"}>
                <div>
                    <label>
                        Фамилия Имя
                        <input
                    ...
                    <label>
                        Ссылка на фото программы
                        <input
                            className={"input"}
                            placeholder={"Ссылка на фото программы"}
                            id={"program"}
                            value={valueProgram}
                            onChange={e => {
                    ...
                    <label>
                        ...
                        <input
                            className={"input"}
                            placeholder={"Кол-во голосов"}
                            id={"votes"}
                            value={valueVotes}
                            onChange={e => {
                                setValueVotes(e.target.value);
                            }}
                        />
                    </label>
                </div>
                ...
                <button className={"actionBtn"} onClick={() => {
                    ...
                    PatchParticipant(e.id, participant)
                }}>Сохранить
                </button>
            </div>
        </div>
